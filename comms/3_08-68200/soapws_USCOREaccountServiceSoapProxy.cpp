...
{	struct soap *soap = this;
	...
	if (!soap_endpoint)
		soap_endpoint = "http://10.194.12.2:8010/ws_account?ver_=0";
...
int ws_USCOREaccountServiceSoapProxy::WhoAmI(_ns1__WhoAmIRequest *ns1__WhoAmIRequest, _ns1__WhoAmIResponse *ns1__WhoAmIResponse)
{	struct soap *soap = this;
	struct __ns1__WhoAmI soap_tmp___ns1__WhoAmI;
	const char *soap_action = NULL;
	if (!soap_endpoint)
		soap_endpoint = "http://10.194.12.2:8010/ws_account?ver_=0";
	soap_action = "ws_account/WhoAmI?ver_=0";
	soap->encodingStyle = NULL;
	soap_tmp___ns1__WhoAmI.ns1__WhoAmIRequest = ns1__WhoAmIRequest;
	soap_begin(soap);
	soap_serializeheader(soap);
	soap_serialize___ns1__WhoAmI(soap, &soap_tmp___ns1__WhoAmI);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put___ns1__WhoAmI(soap, &soap_tmp___ns1__WhoAmI, "-ns1:WhoAmI", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	if (soap_connect(soap, soap_endpoint, soap_action)
	 ...
	 || soap_end_send(soap))
		return soap_closesock(soap);
	if (!ns1__WhoAmIResponse)
		return soap_closesock(soap);
	ns1__WhoAmIResponse->soap_default(soap);
	if (soap_begin_recv(soap)
	 || soap_envelope_begin_in(soap)
	 || soap_recv_header(soap)
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	ns1__WhoAmIResponse->soap_get(soap, "ns1:WhoAmIResponse", "");
	if (soap->error)
	{	if (soap->error == SOAP_TAG_MISMATCH && soap->level == 2)
			return soap_recv_fault(soap);
		return soap_closesock(soap);
