...
CWizardSprayWelcome::CWizardSprayWelcome(CWizardSprayData *data, _U_STRINGorID title) : baseClass(title), m_allowWelcomeToHide(true)
{
	m_data = data;
}

CWizardSprayWelcome::~CWizardSprayWelcome()
{
}
...
void CWizardSprayWelcome::InitializeValues(void)
{
	bool showWelcome = m_data->GetShowWelcome();
	m_buttonSkipWelcome.SetCheck(showWelcome ? BST_UNCHECKED : BST_CHECKED);
}

bool CWizardSprayWelcome::StoreValues(void)
{
	m_data->SetShowWelcome(m_buttonSkipWelcome.GetCheck() == BST_UNCHECKED);
	return true;
}

// Overrides from base class
int CWizardSprayWelcome::OnSetActive()
{
	...
	if(m_allowWelcomeToHide)
	{
		...
		{
			result = m_data->FindFirstPageAfterWelcome();
		}
	}
...
int CWizardSprayWelcome::OnWizardNext()
{
...
void CWizardSprayWelcome::OnHelp()
{
	m_data->ShowHelp(IDD);
}
