...
typedef struct tagNMCTC2ITEMS
{
	...
	POINT   pt;      // Client Coordinates
	// TJ GrETL requirement - allow for not grabbing focus on selection change
	bool	bNoFocusGrab;
...
class ATL_NO_VTABLE CCustomTabCtrl : 
	...
	public WTL::COffscreenDrawRect< T >
{
public:
...
protected:
	...
	typedef WTL::COffscreenDrawRect< T > offscreenDrawClass;
...
public:
	...
	void DoPaint(WTL::CDCHandle dc, RECT &rcClip)
	{
...
public:
	...
	{
		WTL::CImageList imageListOld = m_imageList;
	...
	int InsertItem(int nItem, TItem* pItem, bool bSelectItem = false)
	{
		...
		if( nNewCount==1 )
		{
			pT->SetCurSel(0, true, true); // TJ no focus grab on insertion
		}
		else if(bSelectItem)
		{
			pT->SetCurSel(nItem, true, true); // TJ no focus grab on insertion
		}
		else if (curSel >= nItem)	//GJS If inserting before current sel, reselect current sel.
		{
			pT->SetCurSel(curSel + 1, true, true); // TJ no focus grab on insertion
		}
	...
	*/
	...
	// TJ GrETL requirement - allow for not grabbing focus on selection change
	int SetCurSel(int nItem, bool bNotify = true, bool bNoFocusGrab = false)
	{
		T* pT = static_cast<T*>(this);
		ATLASSERT(::IsWindow(m_hWnd));
