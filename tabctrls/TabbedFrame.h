...
public:
	...
	void SetImageWidth(int imageWidth)  //GJS
	{
		m_cxImage = imageWidth;
	}
	...
	void CalcTabAreaHeight(void)
	{
		...
		if(TabCtrl.IsWindow())
		{
			...
			if(hFont != NULL)
			{
				WTL::CClientDC dc(TabCtrl);
				WTL::CFontHandle hFontOld = dc.SelectFont(hFont);
				TEXTMETRIC tm = {0};
...
public:
	...
	int AddTab(HWND hWnd, LPCTSTR sTabText, int nImageIndex = -1, int pos = -1)  //GJS
	{
		if(hWnd == NULL)
		{
			return -1;
		...
		if(pItem)
		{
			...
			nNewTabIndex = m_TabCtrl.InsertItem(pos == -1 ? nOldCount : pos, pItem);
...
#define CHAIN_ACTIVETABVIEW_CHILD_COMMANDS2(tabClass) \
	...
	{ \
		HWND hWndChild = tabClass.GetActiveView(); \
		if(hWndChild != NULL) \
			lResult = ::SendMessage(hWndChild, uMsg, wParam, 0); \
	}
			...
			if(nNewTab >= 0)
			{
				...
				if(pItem->UsingTabView())
				{
					...
					if( hWndNew != hWndOld )
					{
						...
						LPNMCTC2ITEMS pnm=reinterpret_cast<LPNMCTC2ITEMS>(pnmh);
						if(!pnm->bNoFocusGrab)
							::SetFocus(hWndNew);
					}
				}
...
typedef ATL::CWinTraits<WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN> TabbedChildWindowWinTraits;	//GJS
