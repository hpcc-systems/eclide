...
private:
	...
	CSimpleArray<_CSTRING_NS::CString> m_asList;
	HKEY m_hKey;
...
public:
	...
	CCustomAutoComplete(const HKEY p_hRootKey, const _CSTRING_NS::CString& p_sSubKey)
	{
		InternalInit();
	...
	CCustomAutoComplete(const CSimpleArray<_CSTRING_NS::CString>& p_sItemList)
	{
		InternalInit();
...
public:
	...
	BOOL SetList(const CSimpleArray<_CSTRING_NS::CString>& p_sItemList)
	{
	...
	BOOL SetStorageSubkey(LPCTSTR p_lpszSubKey, HKEY p_hRootKey = HKEY_CURRENT_USER)
	{
		...
		return SetStorageSubkey(p_hRootKey, _CSTRING_NS::CString(p_lpszSubKey));

	}
	
	BOOL SetStorageSubkey(HKEY p_hRootKey, const _CSTRING_NS::CString& p_sSubKey)
	{
	...
	BOOL Bind(HWND p_hWndEdit, DWORD p_dwOptions = 0, LPCTSTR p_lpszFormatString = NULL)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			hr = m_pac->Init(p_hWndEdit, this, NULL, (LPCOLESTR)p_lpszFormatString);
	...
	BOOL AddItem(const _CSTRING_NS::CString& p_sItem)
	{
	...
	{
		return AddItem(_CSTRING_NS::CString(p_lpszItem));
	}
	...
	BOOL SetRecentItem(const _CSTRING_NS::CString& p_sItem)
	{
	...
	BOOL GetRecentItem(_CSTRING_NS::CString& p_sItem)
	{
	...
	BOOL RemoveItem(_CSTRING_NS::CString& p_sItem)
	{
		if (p_sItem.GetLength() != 0)
		{
			if (m_asList.Find(p_sItem) != -1)
	...
	BOOL RemoveItem(LPCTSTR p_lpszItem)
	{
		...
		return RemoveItem(_CSTRING_NS::CString(p_lpszItem));
	}
...
public:
	...
	STDMETHODIMP Next(ULONG celt, LPOLESTR* rgelt, ULONG* pceltFetched)
	{
		...
		for (; i < celt; i++)
		{
			...
			lstrcpyW(rgelt[i], CT2OLE(m_asList[m_nCurrentElement]));
...
private:
	...
	BOOL InitStorage(HKEY p_hRootKey, const _CSTRING_NS::CString& p_sSubKey)
	{
	...
	BOOL RemoveFromStorage(const _CSTRING_NS::CString& p_sItem)
	{
	...
	BOOL LoadFromStorage()
	{
		...
		while (ERROR_SUCCESS == lResult)
		{
			...
			if (ERROR_SUCCESS == lResult)
				m_asList.Add(_CSTRING_NS::CString(szValueName));
	...
	BOOL AddToStorage(const _CSTRING_NS::CString& p_sName)
	{
	...
	BOOL WriteRecentItem(const _CSTRING_NS::CString& p_sItem)
	{
	...
	BOOL ReadRecentItem(_CSTRING_NS::CString& p_sItem)
	{
