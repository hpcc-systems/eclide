...
HWND OpenAttributeMDI(CMainFrame* pFrame, IAttribute * attribute, IWorkspaceItem * workspaceItem, bool bHistoryView=false, 
	const CSyntaxErrorVector & attrSyntax = CSyntaxErrorVector(), Dali::CEclExceptionVector * errors = NULL, const std::_tstring & searchTerm = _T(""), FINDMODE findmode = FINDMODE_NONE, unsigned int row = 0, unsigned int col = 0, unsigned int len = 0);
