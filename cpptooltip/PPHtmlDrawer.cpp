...
SIZE CPPHtmlDrawer::DrawHtmlString (CPPString & sHtml, LPCRECT lpRect)
{
	...
	COLORREF clrShadow = m_bIsEnable ? m_crShadow : GetColorByName(_T(""));//GJS
...
//	pt.y = lpRect->top;
									...
									if (!sParameter.IsEmpty())
									{
										...
										else if (sParameter == _T("color"))
										{
											if (m_bIsEnable)
												...
												m_defStyle.crText = GetColorByName(_T(""));//GJS
										}
								...
								if (!sParameter.IsEmpty())
								{
									...
									else if (sParameter == _T("color"))
									{
										if (m_bIsEnable)
											...
											csTemp.crText = GetColorByName(_T(""));//GJS
									}
								} //if
							} //while
						...
						case TAG_IMAGELIST:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								if (NULL != hBitmap)
								{
									...
									//ENG: Gets a std::max columns and rows of the images on the bitmap
	...
	for (i = nFirstLine; i < m_nCurLine; i++)
	{
		...
		if (0 != m_hline.nAddPercentWidth)
		{
			...
			szTextArea.cx = std::max<LONG>(szTextArea.cx, m_hline.nWidthLine);
		} //if
	} //for
//
//	if (NULL != lpSize)
...
int CPPHtmlDrawer::VerticalAlignText(int y, int nHeight)
{
	...
	if (MODE_FIRSTPASS == m_nNumPass)
	{
		...
		m_hline.nDescentLine = std::max<int>(m_hline.nDescentLine, nHeight - m_tm.tmAscent);
		m_hline.nHeightLine = std::max<int>(m_hline.nHeightLine, m_tm.tmAscent);
	}
	else if (MODE_DRAW == m_nNumPass)
	{
		switch (m_defStyle.nVertAlign)
...
void CPPHtmlDrawer::SetDefaultCssStyles()
{
	...
	str += _T("h3 {font-size: 12pt; text-align:center; color:black; font-family:Arial; font-weight:bold}\r\n");
	str += _T("h4 {font-size: 10pt; text-align:center; color:black; font-family:Verdana; font-weight:bold}\r\n");
			...
			if (0 != chSymbol)
			{
				...
				else if (strName == _T("color"))
				{
					if (m_bIsEnable)
						...
						cs.crText = GetColorByName(_T(""));//GJS
				}
				...
				else if (strName == _T("border-color"))
				{
					if (m_bIsEnable)
						...
						cs.crBorderLight = GetColorByName(_T(""));//GJS
	...
	while (i < sProperties.GetLength())
	{
		...
		else if (sParameter == _T("bordercolordark"))
		{
			if (m_bIsEnable)
				...
				cs.crBorderDark = GetColorByName(_T(""));//GJS
		}
