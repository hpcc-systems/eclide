...
struct IPinnedLabel
{
	...
	public:
		...
		void DrawLabel(WTL::CDC& dc,const CRect& rc,const CSide& side) const
		{
			CRect rcOutput(rc);
	...
	virtual long DesiredWidth(WTL::CDC& dc) const=0;
	...
	virtual void Draw(WTL::CDC& dc,const CRect& rc,const CSide& side) const=0;
...
public:
	...
	virtual long DesiredWidth(WTL::CDC& dc) const
	{
		SIZE sz;
	...
	virtual void Draw(WTL::CDC& dc,const CRect& rc,const CSide& side) const
	{
		dc.Rectangle(&rc);
...
public:
	...
	void DrawPassiveTab(unsigned long i,WTL::CDC& dc,const CRect& rc,const CSide& side) const
	{
		CRect rcOutput(rc);
		rcOutput.DeflateRect(captionPadding,captionPadding);
	...
	void DrawActiveTab(unsigned long i,WTL::CDC& dc,const CRect& rc,const CSide& side) const
	{
...
public:
	...
	bool CalculateRect(WTL::CDC& dc,CRect& rc,long width,long leftPadding,long rightPadding)
	{
		if(IsVisible())
		{
			CopyRect(rc);
	...
	void UpdateLayout(WTL::CDC& dc) const
	{
...
//		long availableWidth=bHorizontal ? Width() : Height();
    ...
    void Draw(WTL::CDC& dc,bool bEraseBackground=true)
    {
		if(IsVisible())
		{
			if(bEraseBackground)
...
public:
	...
	void UpdateLayout(WTL::CDC& dc,CRect& rc)
	{
	...
	void Draw(WTL::CDC& dc)
	{
		EraseBackground(dc);
		for(int i=0;i<barsCount;i++)
			m_bars[i].Draw(dc,false);
	}
	void EraseBackground(WTL::CDC& dc)
	{
		CDWSettings settings;
		CBrush bgrBrush;
		bgrBrush.CreateSolidBrush(settings.CoolCtrlBackgroundColor());
