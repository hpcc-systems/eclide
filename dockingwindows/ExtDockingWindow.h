...
protected:
	...
	struct CButton : CButtonBase
	{
		...
		virtual void Draw (WTL::CDC& dc, int state)=0;
		virtual void Draw (WTL::CDC& dc) { Draw(dc, EBHC_NORMAL); }
	...
	{
	public:
        virtual void Draw(WTL::CDC& dc, int state = EBHC_NORMAL)
        {
            if(m_pTheme && m_pTheme->m_hTheme)
            {
