...
protected:
	...
	struct CButton : CButtonBase
	{
		...
		virtual void Draw (WTL::CDC& dc)=0;
		virtual void Draw (HWND hWnd, WTL::CDC& dc, ButtonState eState)=0;
		virtual void Press(HWND hWnd)
		{
			WTL::CWindowDC dc(hWnd);
	...
	{
	public:
		virtual void Draw(WTL::CDC& dc)
		{
			Draw(dc.WindowFromDC(), dc, eButtonState_Normal);
		}

		virtual void Draw(HWND hWnd, WTL::CDC& dc, ButtonState eState)
		{
			...
			if(settings.CYSmCaption() > nMaxHeightSingleWidth)
			{
				...
				WTL::CFontHandle fontOld = dc.SelectFont(fontClose);
...
public:
	...
	void Draw(HWND hWnd,WTL::CDC& dc)
	{
#ifndef DF_FOCUS_FEATURES
		BOOL bDescendantHasFocus = FALSE;
#else
